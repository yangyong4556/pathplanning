...
%% 结果保存文件夹
save_dir = fullfile(wordingFolder, 'CEC2022_自定义_F1_F2_F3_F4_F5_F6_F7_F8_F9_F10_F11_F12');
...
%% CEC2022 测试函数设置
fun_names = {'F1','F2','F3','F4','F5','F6','F7','F8','F9','F10','F11','F12'};

% CEC2022 各函数理论最优值
fopt = [300, 400, 600, 800, 900, 1800, 2000, 2200, 2300, 2400, 2600, 2700];
...
for i = 1:num_func
    func_id = i;
    [lb, ub, dim_now, fobj] = CEC2022(func_id, dim);

    fprintf('\n====================================================\n');
    fprintf('正在测试 CEC2022 函数：%s\n', fun_names{i});
    fprintf('函数编号：%d | 维度：%d | 理论最优值：%.4f\n', func_id, dim_now, fopt(i));
    fprintf('====================================================\n');
    ...
    for j = 1:num_alg
        ...
        for k = 1:runs
            ...
            if ~isfinite(fit)
                warning('%s 在 %s 第 %d 次运行得到非有限适应度，已记录为 Inf。', alg_name, fun_names{i}, k);
                fit = Inf;
            end
            ...
            fprintf('%s | %s | Run %02d | Fit: %.8e | Error: %.8e | Time: %.6f s\n', ...
                alg_name, fun_names{i}, k, fit, abs(fit - fopt(i)), run_time);
        end
...
%% 保存 MAT 结果
save(fullfile(dataNdSave_dir, 'CEC2022_F1等12个_ResultMat.mat'), ...
    'results', 'times', 'curves', 'fun_names', 'alg_names', 'fopt');
...
file_raw = fullfile(dataNdSave_dir, 'all_fit_results_CEC2022.xlsx');
...
file_error = fullfile(dataNdSave_dir, 'error_to_optimum_CEC2022.xlsx');
...
for i = 1:num_func
    ...
    title(['Convergence curve - CEC2022 ', fun_names{i}]);
    xlabel('Iterations');
    ylabel('Fitness');
    legend(alg_names, 'Location', 'northeast');
    grid on;

    saveas(gcf, fullfile(imgNdSave_dir, ['curve_CEC2022_', fun_names{i}, '.fig']));
    print(gcf, fullfile(imgNdSave_dir, ['curve_CEC2022_', fun_names{i}, '.tif']), '-dtiff', '-r600');
...
fprintf('\n====================================================\n');
fprintf('结果已保存至文件夹：%s\n', save_dir);
fprintf('1）CEC2022_F1等12个_ResultMat.mat\n');
fprintf('2）六种指标统计结果.xlsx\n');
fprintf('3）all_fit_results_CEC2022.xlsx\n');
fprintf('4）error_to_optimum_CEC2022.xlsx\n');
fprintf('5）各测试函数收敛曲线 .fig 和 .tif\n');
fprintf('====================================================\n');